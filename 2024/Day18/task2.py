...
def bfs_shortest_path(grid, start, end):
    ...
    return -1   
def find_first_blocking_byte(grid, corrupted_positions, start, end):
    for i, (x, y) in enumerate(corrupted_positions):
        grid[y][x] = '#'
        if bfs_shortest_path(grid, start, end) == -1:
            return x, y
    return None
...
def main():
    ...
    result = find_first_blocking_byte(grid, corrupted_positions, start, end)
